<?php
	...
	// 12か月分の時 http://viewe-morioka.ddo.jp/dump_csv.php?ymd=2016
	// 31日分の時 http://viewe-morioka.ddo.jp/dump_csv.php?ymd=201611
	// 24時間分の時 http://viewe-morioka.ddo.jp/dump_csv.php?ymd=20161108
	// 突出データを表示しない 2017.02.18
	...
	function write_csvFile($ymdStr, $fileRec)
	{
		...
		if (is_dir($foldaPath))  // csvdataフォルダが有る
		{
			// 古いファイルを消す
			$dir = dir($foldaPath);
			while (FALSE !== ($oldFileName = $dir->read()))
			{
				$fileArray[] = $oldFileName;
			}

			foreach ($fileArray as $oldFileName)
			{
				unlink($foldaPath . "/" . $oldFileName);  // 消す
				//echo $oldFileName . "<br />";  // DEBUG
			}
		}
		else  // フォルダが無い
		{
		...
		$pathNameStr = $foldaPath . "/" . $ymdStr . ".csv";
		$fp = fopen($pathNameStr, 'wb');
		if ($fp)
		{
			if (flock($fp, LOCK_EX))
			{
				...
				{
					// UTF-8のまま
					//echo "iPhone or iPad" . "<br />";
				}
				else  // Mac, Windows, Android
				{
					$fileRec = mb_convert_encoding($fileRec, 'Shift_JIS', 'UTF-8');  // Shift_JISに変換
				}
				if (fwrite($fp,  $fileRec) === FALSE)
				{
					writeLog2("[dump_csv] CSVファイルの書き込みに失敗しました。");
					echo "ERROR 011";
				}
				else
				{
					//echo ログファイルを書き込みました。."<br />\n";
					echo $pathNameStr;  // 正常終了の時はパス名を返す
					
					header('Content-Type: application/force-download');
					header('Content-Length: '.filesize($pathNameStr));
					$fname = $ymdStr . ".csv";
					header('Content-disposition: attachment; filename="'.$fname.'"');
					readfile($pathNameStr);
					exit;
				}
				flock($fp, LOCK_UN);
			}
			else  // if (flock($fp, LOCK_EX))
			{
				...
				writeLog2("[dump_csv] CSVファイルのロックに失敗しました。");
				echo "ERROR 012";
			}
			fclose($fp);
		}
		else
		{
			...
			writeLog2("[dump_csv] CSVファイルのfopenに失敗しました。");
			echo "ERROR 013";
		}
	}
